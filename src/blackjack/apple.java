...
public class ascoltatore implements ActionListener {  //eredito il metodo ActionPerformed
	...
	public void cpu(){   // metodo cpu simula la giocata da parte della cpu tramite un semplice while
		cpu=(int)(Math.random()*12)+1;
		cpu=control(cpu);
		x=(int)(Math.random()*12)+1;
		x=control(x);
		cpu+=x;
		while (cpu<player){  // tramite questa istruzione il programma non fa altro ke generare un numero rand finche la somma dei valori usciti non supera quella della giocatore
			x=(int)(Math.random()*12)+1;
			cpu+=control(x);
		}
		if (cpu>21){
