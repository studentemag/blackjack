...
public class TestCpu extends TestCase {
	...
	public final void testCpu01easymock() {
		...
		apple.setPlayer(playerIn);
		apple.setState_player(state_playerIn);
		apple.setState_cpu(state_cpuIn);
		
		//Invocazione del metodo sotto test
		apple.cpu();
		
		//Verifichiamo il corretto aggiornamento dei punteggi globali
		assertEquals("Punteggio globale Player non corretto!", state_playerExp, apple.getState_player());
		assertEquals("Punteggio globale CPU non corretto!", state_cpuExp, apple.getState_cpu());
		
		//Verifichiamo che l'esito della partita sia visualizato correttamente
		assertEquals("Campo field non corretto!", fieldExp, apple.getFieldText());
		
		//Verifichiamo che i nuovi punteggi globali siano visualizzati correttamente
		assertEquals("Visualizzazione punteggio globale Player non corretto!", state_playerExp, BjOracle.playerGlobalScore((apple.getArea_scoreText())));
		assertEquals("Visualizzazione punteggio globale CPU non corretto!", state_cpuExp, BjOracle.cpuGlobalScore(apple.getArea_scoreText()));
	}
